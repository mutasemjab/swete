...
<div class="card mb-5">
    ...
    <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div>
            <label class="form-label">{{ __('app.name_en') }}</label>
            <input type="text" name="entity_name_en" value="{{ old('entity_name_en', $tender?->entity_name_en) }}"
                dir="ltr" class="form-input @error('entity_name_en') is-invalid @enderror">
            @error('entity_name_en')
                <p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="sm:col-span-2">
            ...
            <div class="flex items-start gap-2">
                <div class="flex-1">
                    <select id="tender_party_id" name="party_id"
                        class="js-select2 form-select @error('party_id') is-invalid @enderror">
                        <option value="">{{ __('app.select') }}</option>
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->id }}" @selected(old('party_id', $tender?->party_id) == $customer->id)>
                                {{ $customer->localized_name }} ({{ $customer->code }})</option>
                        @endforeach
                    </select>
                    @error('party_id')
                        <p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>
        ...
        <div x-data="{ scope: '{{ old('location_scope', $tender?->location_scope ?? 'inside_jordan') }}' }" class="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-5">
            @if ($type === 'tender')
                <div>
                    <label class="form-label">{{ __('tenders.tender_location_scope') }} <span
                            class="text-rose-500">*</span></label>
                    <select name="location_scope" x-model="scope"
                        class="form-select @error('location_scope') is-invalid @enderror">
            ...
            <div x-show="scope === 'inside_jordan'">
                ...
                <select name="governorate" class="js-select2 form-select @error('governorate') is-invalid @enderror">
                    ...
                    @foreach (\App\Models\Tender::JORDAN_GOVERNORATES as $key => $names)
                        <option value="{{ $key }}" @selected(old('governorate', $tender?->governorate) === $key)>
                            {{ $names[app()->getLocale() === 'en' ? 'en' : 'ar'] }}</option>
                    @endforeach
                </select>
                @error('governorate')
                    <p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>
        ...
        <div>
            <label class="form-label">
                {{ $isService ? __('tenders.service_win_probability') : __('tenders.tender_win_probability') }}
            </label>
            <div class="relative">
                <input type="number" name="win_probability"
                    value="{{ old('win_probability', $tender?->win_probability) }}" min="0" max="100"
                    dir="ltr" class="form-input pe-9 @error('win_probability') is-invalid @enderror">
                <span class="absolute top-1/2 -translate-y-1/2 end-3.5 text-slate-400 text-sm">%</span>
            </div>
            @error('win_probability')
                <p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="date" name="submission_deadline"
                value="{{ old('submission_deadline', $tender?->submission_deadline?->toDateString()) }}"
        ...
        <div>
            ...
            <select name="status" class="form-select @error('status') is-invalid @enderror">
                @foreach (['open', 'closed', 'won', 'lost'] as $status)
                    <option value="{{ $status }}" @selected(old('status', $tender?->status ?? 'open') === $status)>
                        {{ __('tenders.tender_status_' . $status) }}</option>
                @endforeach
            </select>
            @error('status')
                <p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>
