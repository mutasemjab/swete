...
@section('title', __('tenders.quote_supply_scopes_list'))
@section('breadcrumb', __('tenders.quote_supply_scopes_list'))

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">{{ __('tenders.quote_supply_scopes_list') }}</h1>
        <p class="page-subtitle">{{ __('tenders.quote_supply_scopes_subtitle') }}</p>
    </div>
    <a href="{{ route('quote-supply-scopes.create') }}" class="btn-primary">
...
<div class="card overflow-hidden">
    @if($scopes->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-slate-800 font-bold text-lg">{{ __('tenders.no_quote_supply_scopes') }}</p>
        </div>
    @else
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('quote-supply-scopes.edit', $item) }}"
                                ...
                                <button type="button" title="{{ __('app.delete') }}"
                                        @click="$dispatch('delete-confirm', { action: '{{ route('quote-supply-scopes.destroy', $item) }}', message: '{{ __('app.delete_confirm_msg') }}' })"
