                ...
                <div x-show="scope === 'inside_jordan'">
                    ...
                    <select name="governorate_id" class="js-select2 form-select @error('governorate_id') is-invalid @enderror">
                        ...
                        @foreach($governorates as $governorate)
                            <option value="{{ $governorate->id }}" @selected(old('governorate_id', $purchaseRequest?->governorate_id) == $governorate->id)>{{ $governorate->localized_name }}</option>
                        @endforeach
                    </select>
    ...
    <div class="card mb-5">
        <div class="card-header">
            <h3 class="font-bold text-slate-700 flex items-center gap-2">
                ...
                {{ __('external_purchases.attachments') }}
            </h3>
        ...
        <div class="px-6 py-5 space-y-3">
            ...
            <p x-show="attachments.length === 0" class="text-sm text-slate-400">{{ __('external_purchases.no_attachments') }}</p>
        </div>
    </div>
</div>
