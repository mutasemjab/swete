...
@section('title', __('accounting.customer_groups'))
@section('breadcrumb', __('accounting.customer_groups'))

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">{{ __('accounting.customer_groups') }}</h1>
        <p class="page-subtitle">{{ __('accounting.group_name') }}</p>
    </div>
    <a href="{{ route('accounting.customer-groups.create') }}" class="btn-primary">
        <i class="fa-solid fa-plus"></i>
        {{ __('accounting.add_group_customer') }}
    </a>
</div>
...
<div class="card overflow-hidden">
    @if($groups->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <a href="{{ route('accounting.customer-groups.create') }}" class="btn-primary mt-5">
                <i class="fa-solid fa-plus"></i>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('accounting.customer-groups.edit', $group) }}"
                                ...
                                <button type="button" title="{{ __('app.delete') }}"
                                        @click="$dispatch('delete-confirm', { action: '{{ route('accounting.customer-groups.destroy', $group) }}', message: '{{ __('app.delete_confirm_msg') }}' })"
