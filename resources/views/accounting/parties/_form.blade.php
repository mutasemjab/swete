@php $party = $party ?? null; @endphp
<div class="card mb-5">
    <div class="card-header">
        <h3 class="font-bold text-slate-700 flex items-center gap-2">
            <i class="fa-solid fa-{{ $type === 'customer' ? 'address-book' : 'truck' }} text-blue-500 text-sm"></i>
            {{ __('accounting.' . $type) }}
        </h3>
    </div>
    <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <input type="text" name="name_en" value="{{ old('name_en', $party?->name_en) }}" dir="ltr"
        ...
        <div>
            ...
            <select name="party_group_id" class="form-select @error('party_group_id') is-invalid @enderror">
                <option value="">{{ __('accounting.no_parent_group') }}</option>
                @foreach($groups as $group)
                    <option value="{{ $group->id }}" @selected(old('party_group_id', $party?->party_group_id) == $group->id)>{{ $group->path }}</option>
                @endforeach
            </select>
            @error('party_group_id')<p class="form-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="form-label">{{ __('accounting.party_phone') }}</label>
            <input type="text" name="phone" value="{{ old('phone', $party?->phone) }}" dir="ltr"
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ old('email', $party?->email) }}" dir="ltr"
        ...
        <div>
            ...
            <input type="text" name="tax_number" value="{{ old('tax_number', $party?->tax_number) }}" dir="ltr"
        ...
        <div class="sm:col-span-2">
            ...
            <textarea name="address" rows="2" class="form-input @error('address') is-invalid @enderror">{{ old('address', $party?->address) }}</textarea>
        ...
        <div>
            ...
            <input type="number" name="opening_balance" value="{{ old('opening_balance', $party?->opening_balance ?? 0) }}"
        ...
        <div class="flex items-center">
            <label class="relative inline-flex items-center cursor-pointer" dir="ltr">
                ...
                <input type="checkbox" name="status" value="1" class="sr-only peer"
                       @checked(old('status', $party?->status ?? true))>
