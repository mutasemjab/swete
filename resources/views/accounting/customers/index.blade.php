...
@section('title', __('accounting.customers_list'))
@section('breadcrumb', __('accounting.customers'))

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">{{ __('accounting.customers_list') }}</h1>
        <p class="page-subtitle">{{ __('accounting.customers_subtitle') }}</p>
    </div>
    <a href="{{ route('accounting.customers.create') }}" class="btn-primary">
...
<div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
    <div class="card px-5 py-4 flex items-center gap-4">
        ...
        <div>
            <p class="text-2xl font-black text-slate-800">{{ $customers->total() }}</p>
            <p class="text-xs text-slate-500 font-medium mt-0.5">{{ __('accounting.total_customers') }}</p>
        </div>
    </div>
</div>

<form method="GET" action="{{ route('accounting.customers.index') }}" class="card px-5 py-4 mb-4 flex flex-wrap gap-3">
...
<div class="card overflow-hidden">
    @if($customers->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-slate-800 font-bold text-lg">{{ __('accounting.no_customers') }}</p>
            <a href="{{ route('accounting.customers.create') }}" class="btn-primary mt-5">
                <i class="fa-solid fa-plus"></i>
    ...
    @else
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-5 py-4">
                            <span class="font-mono font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded-lg text-sm">{{ $customer->code }}</span>
                        </td>
                        <td class="px-5 py-4"><p class="font-bold text-slate-800">{{ $customer->localized_name }}</p></td>
                        <td class="px-5 py-4 hidden md:table-cell text-sm text-slate-600">{{ $customer->group?->localized_name ?? '—' }}</td>
                        <td class="px-5 py-4 hidden lg:table-cell text-sm text-slate-600" dir="ltr">{{ $customer->phone ?? '—' }}</td>
                        <td class="px-5 py-4">
                            @if($customer->status)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('accounting.customers.edit', $customer) }}"
                                ...
                                <button type="button" title="{{ __('app.delete') }}"
                                        @click="$dispatch('delete-confirm', { action: '{{ route('accounting.customers.destroy', $customer) }}', message: '{{ __('app.delete_confirm_msg') }}' })"
        ...
        <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between gap-4 flex-wrap">
            <p class="text-sm text-slate-500">
                {{ __('app.showing') }} <span class="font-bold text-slate-700">{{ $customers->firstItem() }}</span>
                {{ __('app.to') }} <span class="font-bold text-slate-700">{{ $customers->lastItem() }}</span>
                {{ __('app.of') }} <span class="font-bold text-slate-700">{{ $customers->total() }}</span>
                {{ __('app.results') }}
            </p>
            <div class="flex gap-1">
                ...
                @else
                    <a href="{{ $customers->previousPageUrl() }}" class="btn-secondary btn-sm">{{ __('app.previous') }}</a>
                @endif
                @if($customers->hasMorePages())
                    <a href="{{ $customers->nextPageUrl() }}" class="btn-primary btn-sm">{{ __('app.next') }}</a>
                @else
