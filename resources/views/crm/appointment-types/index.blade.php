...
@section('title', __('crm.appointment_types_list'))
@section('breadcrumb', __('crm.appointment_types_list'))

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">{{ __('crm.appointment_types_list') }}</h1>
        <p class="page-subtitle">{{ __('crm.appointment_types_subtitle') }}</p>
    </div>
    <a href="{{ route('appointment-types.create') }}" class="btn-primary">
        <i class="fa-solid fa-plus"></i>
        {{ __('crm.add_appointment_type') }}
    </a>
</div>
...
<div class="card overflow-hidden">
    @if($types->isEmpty())
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-slate-800 font-bold text-lg">{{ __('crm.no_appointment_types') }}</p>
        </div>
    @else
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('appointment-types.edit', $item) }}"
                                ...
                                <button type="button" title="{{ __('app.delete') }}"
                                        @click="$dispatch('delete-confirm', { action: '{{ route('appointment-types.destroy', $item) }}', message: '{{ __('app.delete_confirm_msg') }}' })"
